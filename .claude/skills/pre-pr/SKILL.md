...
description: Use after implementation is complete — spawns review and simplification agents in a loop until both pass, then presents final work to user. Do NOT prompt the user until the pipeline completes.
---
...
Automated quality gate that runs before presenting work. Spawns agents for review and simplification, loops until both pass cleanly.
...
- Read the agent definition at `.claude/agents/code-reviewer.md`
- Use the Task tool to spawn a `general-purpose` agent
- Pass the contents of `.claude/agents/code-reviewer.md` as the prompt
- If the agent returned fixes: go to Step 1 (re-review after fixes)
- If "PASS": continue to Step 2

### Step 2: Simplify (agent)

- Read the agent definition at `.claude/agents/code-simplifier.md`
- Use the Task tool to spawn a `general-purpose` agent
- Pass the contents of `.claude/agents/code-simplifier.md` as the prompt
- If the agent made changes: go to Step 1 (simplification may introduce issues)
- If "PASS": continue to Step 3

### Step 3: Final Review (agent)

- Spawn the same review agent as Step 1 one last time
...
- Agents read the full skill files without bloating the main conversation
- If an agent fails or times out, the orchestrator can retry or present partial results
...
**Always:**
- Use agents for review and simplification passes
